...
mod tests {
    ...
    fn concat_rejects_mismatched_sample_rates() {
        let a = create_valid_16bit_wav();
        let mut b = create_valid_16bit_wav();
        b[24..28].copy_from_slice(&44100u32.to_le_bytes());
        assert!(concat_wav_files(vec![a, b]).is_err(),
            "mismatched-rate concat must error, not stamp the first header over foreign PCM");
    }

    #[test]
    fn duration_and_envelope_survive_truncated_stream() {
        let mut f = create_valid_16bit_wav();
        f.truncate(f.len() - 10);
        assert!(parse_wav_header(&f).is_ok());
        assert!(extract_envelope(&f, 10).is_ok());
    }
}
