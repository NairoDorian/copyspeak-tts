...
/// Mirrors `CliTtsBackend::engine_dir` so uv-managed engine projects resolve
/// consistently for both the one-shot CLI path and the persistent HTTP server.
...
/// Build the (program, leading_args) needed to launch an engine's persistent
/// HTTP server script.
///
/// For uv-managed engines this mirrors the working one-shot CLI path:
///   `uv run --project <engine_dir>/<engine> python <server_script> ...`
/// so the server runs inside the engine's own venv (deps already installed by
/// the installer). For plain `python`/`py` commands it runs the dev/exe-bundled
/// script directly.
fn build_server_launch(engine: &str, user_command: &str) -> Option<(String, Vec<String>)> {
    let is_uv = user_command.eq_ignore_ascii_case("uv") || user_command.to_lowercase().contains("uv");
    if is_uv {
        let ed = engine_dir();
        let project = format!("{ed}/{engine}");
        // Prefer the HTTP server script the installer placed in the uv project.
        let project_script = format!("{ed}/{engine}/scripts/{engine}_server.py");
        // Fall back to the dev script shipped next to the binary/manifest.
        let dev_script = resolve_server_script(&format!("{engine}_server.py"));
        let script = if Path::new(&project_script).exists() {
            project_script
        } else if let Some(dev) = dev_script {
    ...
    let python = resolve_python_command(user_command);
    let script = resolve_server_script(&format!("{engine}_server.py"))?;
    Some((python, vec![script.to_string_lossy().into_owned()]))
}
...
fn spawn_start_thread(
    ...
    std::thread::spawn(move || {
        ...
        let (program, prefix) = match build_server_launch(&engine, &command) {
            Some(x) => x,
            None => {
                log::warn!(
        ...
        log::info!(
            "[LocalServer] Starting {} server on port {} — command: {}",
            engine,
            port,
            cmd_display
        );
...
mod tests {
    ...
    #[test]
    fn uv_launch_mirrors_cli_one_shot_path() {
        let (program, args) = build_server_launch("kitten", "uv").expect("uv launch");
        assert_eq!(program, "uv");
        // uv run --project <engine_dir>/kitten python <...>/kitten_server.py
        let joined = args.join(" ");
        assert!(joined.contains("run --project"), "missing --project: {joined}");
        assert!(joined.contains("/kitten"), "missing engine subdir: {joined}");
        assert!(joined.contains("python"), "missing python: {joined}");
        assert!(
            joined.ends_with("kitten_server.py"),
            "missing server script: {joined}"
        );
    }

    #[test]
    fn python_launch_uses_manifest_script() {
        // In cargo test CARGO_MANIFEST_DIR is src-tauri/, which ships the
        // dev server scripts (e.g. kitten_server.py).
        let (program, args) =
            build_server_launch("kitten", "python").expect("python launch");
        assert_eq!(program, "python");
        assert!(
            args[0].ends_with("kitten_server.py"),
